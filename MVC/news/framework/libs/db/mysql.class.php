...
class mysql {
	...
	function connect ($config) {
		...
		if (!($this->con)) {  // mysqli_connect 连接数据库函数
			$this->err(mysqli_error($this->con));
		}
		if (!mysqli_select_db($this->con, $dbname)) {  // mysqli_select_db 选择库的函数
			$this->err(mysqli_error($this->con));
		}

		mysqli_query($this->con, 'set names ' . $dbcharset);  // 使用 mysqli_query 设置编码格式：mysqli_query("set names utf8")
	}
	...
	function query($sql) {
		// $con = $this;
		if (!($query = mysqli_query($this->con, $sql))) {  // 使用 mysqli_query 函数执行 sql 语句
			$this->err($sql."<br />" . mysqli_error($this->con));  // mysqli_error 报错
		} else {
			return $query;
		}
	}
	...
	************************************************************/
	...
	function insert ($table, $arr) {
		...
		foreach ($arr as $key=>$value) {  // foreach 循环数组
			$value = mysqli_real_escape_string($this->con, $value);  // 对输入的字符串进行转义
		...
		return mysqli_insert_id($this->con);  // 返回上一步 INSERT 操作产生的 ID
 	}
